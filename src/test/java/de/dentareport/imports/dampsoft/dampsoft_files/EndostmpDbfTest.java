...
// TODO: TEST?
public class EndostmpDbfTest {

    private EndostmpDbf endostmpDbf;

    @BeforeEach
    public void setUp() {
        endostmpDbf = new EndostmpDbf();
    }
    ...
    public void it_converts_row_of_data(@Mocked DateConvert mockDateConvert) {
        ...
        List<DbRow> converted = endostmpDbf.convert(dbfRow);
        ...
        assertThat(converted.get(0).value("canal")).isEqualTo("FooBar");
        assertThat(converted.get(0).value("iso2")).isEqualTo("  BizBaz  ");
    }
    ...
    public void it_validates_row_of_data() {
        ...
        assertThat(endostmpDbf.isValidRow(row1)).isTrue();
        assertThat(endostmpDbf.isValidRow(row2)).isTrue();
        assertThat(endostmpDbf.isValidRow(row3)).isFalse();
    }

    private DbfRow testRow() {
        DbfRow dbfRow = new DbfRow();
        dbfRow.addCell(new DbfCell("PATNR", " 234 "));
        dbfRow.addCell(new DbfCell("ZAHN", "12"));
        dbfRow.addCell(new DbfCell("DATUM", "somedate"));
        dbfRow.addCell(new DbfCell("KANAL", "  FooBar  "));
        dbfRow.addCell(new DbfCell("ISO2", "  BizBaz  "));
        return dbfRow;
    }
}