...
public class EndstaendigkeitTest {
    ...
    public void it_evaluates_data(@Mocked Evaluation mockEvaluation,
        ...
        new Expectations() {{
            ...
            result = true;
            mockEventInterface.endstaendigkeit("foo");
            result = "bar";
        }};

        EvaluationColumn column = new Column(mockEvaluation, "endstaendigkeit__of__foo_event").instance();
        CaseData result = column.evaluate(mockRawData, caseData);

        assertThat(result.string("endstaendigkeit__of__foo_event")).isEqualTo("bar");
    }

    @Test
    public void it_gets_required_columns(@Mocked Evaluation mockEvaluation) {
        EvaluationColumn column = new Endstaendigkeit(mockEvaluation, ImmutableMap.of(OF, "foo_event"));
