...
// TODO: TEST?
public class ToothcontactsTest {
    ...
    public void it_evaluates_data(@Mocked Evaluation mockEvaluation,
        ...
        new Expectations() {{
            ...
            result = true;
            mockEventInterface.toothcontacts("foo");
            result = "bar";
        }};

        EvaluationColumn column = new Column(mockEvaluation, "toothcontacts__of__foo_event").instance();
        CaseData result = column.evaluate(mockRawData, caseData);

        assertThat(result.string("toothcontacts__of__foo_event")).isEqualTo("bar");
    }

    @Test
    public void it_gets_required_columns(@Mocked Evaluation mockEvaluation) {
        EvaluationColumn column = new Toothcontacts(mockEvaluation, ImmutableMap.of(OF, "foo_event"));
