...
// TODO: TEST?
public class ToothcountInJawTest {
    ...
    public void it_evaluates_data(@Mocked Evaluation mockEvaluation,
        ...
        new Expectations() {{
            ...
            result = true;
            mockEventInterface.toothcountInJaw("foo");
            result = "bar";
        }};

        EvaluationColumn column = new Column(mockEvaluation, "toothcount_in_jaw__of__foo_event").instance();
        CaseData result = column.evaluate(mockRawData, caseData);

        assertThat(result.string("toothcount_in_jaw__of__foo_event")).isEqualTo("bar");
    }

    @Test
    public void it_gets_required_columns(@Mocked Evaluation mockEvaluation) {
        EvaluationColumn column = new ToothcountInJaw(mockEvaluation, ImmutableMap.of(OF, "foo_event"));
