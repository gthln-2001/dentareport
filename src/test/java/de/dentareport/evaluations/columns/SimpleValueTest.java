...
public class SimpleValueTest {
    ...
    public void it_evaluates_data(@Mocked Evaluation mockEvaluation,
        ...
        new Expectations() {{
            ...
            result = true;
            mockEventInterface.billingcode();
            result = "some-result";
        }};

        EvaluationColumn column = new Column(mockEvaluation, "billingcode__of__foo_event").instance();
        CaseData result = column.evaluate(mockRawData, caseData);

        assertThat(result.string("billingcode__of__foo_event")).isEqualTo("some-result");
    }

    @Test
    public void it_gets_required_columns(@Mocked Evaluation mockEvaluation) {
        EvaluationColumn column = new Billingcode(mockEvaluation, ImmutableMap.of(OF, "foo_event"));
