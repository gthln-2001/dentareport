...
public class CaseCountForPatientUpperJawTest {
    ...
    public void it_evaluates_data(@Mocked Evaluation mockEvaluation,
        ...
        new Expectations() {{
            mockRawData.caseCountUpperJaw();
            result = "23";
        }};
        EvaluationColumn column = new CaseCountForPatientUpperJaw(mockEvaluation, options());

        CaseData result = column.evaluate(mockRawData, testCase());

        assertThat(result.string("case_count_for_patient_upper_jaw")).isEqualTo("23");
    }
}